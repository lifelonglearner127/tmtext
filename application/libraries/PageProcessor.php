...
//			'Long_Description' => $description_long,
	...
	public function process_amazon(){
		...
		if (empty($description)) {
			foreach($this->nokogiri->get('#featurebullets_feature_div ul li span') as $item) {
				$line = trim($item['#text'][0]);
				if (!empty($line)) {
		...
		foreach($this->nokogiri->get('.content .productDescriptionWrapper ul li b') as $item) {
			$line = trim($item['#text'][0]);
			if (!empty($line)) {
				$description[] = $line;
			}
		...
		foreach($this->nokogiri->get('.content .productDescriptionWrapper ul li') as $item) {
			$line = trim($item['#text'][0]);
			if (!empty($line)) {
				$description[] = $line;
			}
