...
class Measure extends MY_Controller {
    ...
    public function update_rec_week_day() {
        ...
        $res = $this->webshoots_model->changeHomePageRecipients($id, $week_day);
        $this->output->set_content_type('application/json')->set_output(json_encode($res));
    }
    ...
    public function send_recipient_category_snaps_report() {
        ...
        $reports = $this->department_members_model->getDepReportsByUserId($uid);
    ...
    public function send_recipient_report_selected_cat() {
        $uid = $this->ion_auth->get_user_id();
...
//    }
    ...
    public function send_dep_snaps_report() {
        $this->load->model('department_members_model');
        $this->load->model('webshoots_model');
        $this->load->model('settings_model');
        $uid = $this->ion_auth->get_user_id();
        $rep_ids = $this->input->post('rep_ids');
        $reports = $this->department_members_model->getDepReportsByIds($rep_ids);
    ...
    public function get_board_view_item_data() {
        ...
        $d = $this->department_members_model->getDataByDepMemberId($dm_id);
        $data['data'] = $d;
        $this->load->view('measure/get_board_view_item_data', $data);        
    }
...
//        }
    ...
    function matches_count($im_data_id) {
        ...
        if(count($matched_sites)==0){
               ...
               $customer ='';
               $data_similar = $this->imported_data_parsed_model->getByImId($im_data_id);
...
//            }
            if (empty($same_pr) && !isset($data_import['parsed_attributes']['model'])) {
               ...
               if ($model=$this->imported_data_parsed_model->check_if_exists_custom_model($im_data_id)) {
                     ...
                     $same_pr = $same_pr = $this->imported_data_parsed_model->getByParsedAttributes($model, 0,$im_data_id );
