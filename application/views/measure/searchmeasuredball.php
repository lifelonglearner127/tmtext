		...
		$(document).ready(function () {
	        ...
	        $(document).on("click", '#products li', function() {
                ...
                startMeasureCompare(im_data_id);
                   ...
                   startMeasureCompareforbatches(im_data_id);
                }//max
...
//max
                 function startMeasureCompareforbatches(id){
                    var searcher = $.post(editorSearchBaseUrl, {  research_data_id: id}, 'html').done(function(data) {
	...
	<div id='an_sort_search_box' class='boxes_content' style='margin-bottom: 15px;'>
		...
		<ul id="products" class='products_an_search'>
			<?php foreach($search_results as $k => $v) { ?>
                                <li data-status='standart' data-value="<?php if(!$batches){ echo $v['imported_data_id'];}else{ echo $v['id'] ; } ?>">
