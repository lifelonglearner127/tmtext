...
class SaksfifthavenueProductsSpider(BaseProductsSpider):
    ...
    allowed_domains = ["saksfifthavenue.com", "recs.richrelevance.com", "saksfifthavenue.ugc.bazaarvoice.com"]
    start_urls = []
    SEARCH_URL = (
    ...
    def parse_product(self, response):
        ...
        cond_set(product, 'title', response.xpath(
            "//header/h2[contains(@class,'short-description')]/text()").extract())
        cond_set(product, 'brand', response.xpath(
            "//header/h1[contains(@class,'brand-name')]/a/text()").extract(),
            conv=string.strip)
        jtext = response.xpath("//script[contains(text(),'var mlrs =')]/text()").re('var mlrs = (.*)')
        # print "JTEXT=",jtext
        jdata = json.loads(jtext[0])
        pprint(jdata)

        tprice = jdata['response']['body']['main_products'][0]['price']['sale_price']
        ...
        description = jdata['response']['body']['main_products'][0]['description']
        cond_set_value(product, 'description', description)

        media = jdata['response']['body']['main_products'][0]['media']
        image_url = "http:" + media['images_server_url'] + media['images_path'] + media['images']['product_detail_image']
        ...
        if url:
            return Request(url, meta=response.meta.copy(), callback=self._parse_rr)
        return product
    ...
    def _gen_rr_request(self, response):
        ...
        product = response.meta['product']
        api = response.xpath("//script[contains(text(),'R3_COMMON.setApiKey')]/text()").re(r"R3_COMMON\.setApiKey\('(.*)'\)")
        if api:
            api = api[0]
            ...
            url = (
                ...
                "&l=1").format(api=api, prodno=product['upc'], ts=int(jsGetTime()))
            return url
    ...
    def _parse_rr(self, response):
        ...
        bvurl = response.meta['bvurl']
        return Request(bvurl,meta=response.meta.copy(),callback=self._parse_bv)
        #return product
                ...
                for pt in m2:
                    ...
                    for iitem in ilist:
                        ...
                        if ilink:
                            ...
                            # name
                            iname = iitem.xpath(
                                "a/div[@class='rr_item_text']/div[@class='medium']/text()").extract()
                            if iname:
                                iname = iname[0]
    ...
    def _gen_bv_url(self,response):
        product = response.meta['product']
        # FIXME: insert productno in url
        url = "http://saksfifthavenue.ugc.bazaarvoice.com/5000-en_us/0475504779569/reviews.djs?format=embeddedhtml"
        return url

    def _parse_bv(self,response):
        product = response.meta['product']
        print "PARSE_BAZAARVOICE"
        text = response.body_as_unicode().encode('utf-8')
        print text
        if response.status == 200:
            ...
            if x:
                ...
                with open("/tmp/saks-bv.html", "w") as f:
                    print "TYPR=",type(html)
                    f.write(html.encode('utf-8'))

                sel = Selector(text=html.encode('utf-8'))

                # FIXME: avrg,total,distr
                average = sel.xpath(
                    "//div[@class='BVRRCustomRating']"
                    "/div[@class='BVRRReviewRatingsContainer']/"
                    "/div[@id='BVRRRatingOverall_Review_Display']"
                    "/div[contains(@class,'BVRRRatingNormalOutOf')]"
                    "/span[contains(@class,'BVRRRatingNumber')]"
                    "/text()"
                    ).extract()
                print "AVRG=", len(average),average
                if average:
                    try:
                ...
                for ih in hist:
                    ...
                    try:
                        ...
                        pass
                print "\n\nDISTR=",distribution,"total/avrg=",total,avrg
                if distribution:
