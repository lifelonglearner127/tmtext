...
class WalmartCaShelfPagesSpider(WalmartCaProductsSpider):
    ...
    def _scrape_product_links(self, response):
        urls = response.xpath(
            '//*[contains(@id, "shelf-thumbs")]'
            '//article/div[contains(@class, "details")]//h1/a/@href').extract()

        urls = [urlparse.urljoin(response.url, x) for x in urls]

        # parse shelf category
        shelf_categories = [c.strip() for c in response.css('#breadcrumb ul li ::text').extract()
                            if len(c.strip()) > 1]
