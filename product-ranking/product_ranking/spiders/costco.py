...
class CostcoProductsSpider(BaseProductsSpider):
    ...
    def parse_product(self, response):
        ...
        model = response.xpath('//div[@id="product-tab1"]//text()').re('Model[\W\w\s]*')
        if len(model) > 0:
        ...
        tab2 = ''.join(response.xpath('//div[@id="product-tab2"]//text()').extract()).strip()
        brand = ''
        for i in tab2.split('\n'):
            if 'Brand' in i.strip():
                brand = i.strip()
        ...
        #price = response.xpath('//div[@class="your-price"]/span[@class="currency"]/text()').extract()
        price = response.xpath('//input[contains(@name,"price")]/@value').extract()
    ...
    def _scrape_total_matches(self, sel):
        try:
            count = sel.xpath('//*[@id="secondary_content_wrapper"]/div/p/span/text()').re('(\d+)')[-1]
            if count:
                return int(count)
            return 0
        except IndexError:
    ...
    def _scrape_product_links(self, sel):
        for link in sel.xpath('//div[contains(@class,"product-tile-image-container")]/a/@href').extract():
    ...
    def _scrape_next_results_page_link(self, response):
        ...
        new_paras = {}
        for key in paras: new_paras[key] = paras[key][0]
        new_paras['currentPage'] = int(new_paras.get("currentPage", 1)) + 1
        link = urlparse.urlunsplit((url_parse.scheme, url_parse.netloc,
                                     url_parse.path, urllib.urlencode(new_paras),
                                     url_parse.fragment))
        return link
