...
class Pet360Scraper(Scraper):

    ##########################################
    ############### PREP
    ##########################################

    INVALID_URL_MESSAGE = 'Expected URL format is http://www.pet360.com/product/<prod-id>/<prod-name>'

    def check_url_format(self):
        if re.match('^http://www\.pet360\.com/product/\d+/.+$', self.product_page_url):
            return True
        return False
    ...
    def _features(self):
        ...
        if features:
            return map(lambda f: self._clean_text(f), features)
