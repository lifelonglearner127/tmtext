    ...
    def put(self, sqs_message, s3_content):
        ...
        try:
            if isinstance(sqs_message, basestring):
                ...
                sqs_message_json = json.loads(sqs_message)
...
#                bucket = boto.connect_s3().create_bucket('contentanalytcis.inc.ch.s3')  # bucket names must be unique
                key = bucket.new_key(str(sqs_message_json["site_id"]) + "/" + str(sqs_message_json["product_id"]))
    ...
    def get(self, timeout=None):
        if self.currentM is None:
            ...
            s3 = S3Connection('AKIAJPOFQWU54DCMDKLQ', '/aebM4IZ97NEwVnfS6Jys6sKVvDXa6eDZsB2X7gP')
            key = s3.get_bucket('contentanalytcis.inc.ch.s3').get_key(str(sqs_message_json["site_id"]) + "/" + str(sqs_message_json["product_id"]))
#            s3 = boto.connect_s3()
#            key = s3.get_bucket('contentanalytcis.inc.ch.s3').get_key(str(sqs_message_json["site_id"]) + "/" + str(sqs_message_json["product_id"]) + "/" + sqs_message_json["uuid"])
            s3_content = key.get_contents_as_string()
            s3.get_bucket('contentanalytcis.inc.ch.s3').delete_key(key)
            return s3_content
        else:
