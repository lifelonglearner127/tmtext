...
class BloomingdalesScraper(Scraper):
    ...
    INVALID_URL_MESSAGE = "Expected URL format is http://www1.bloomingdales.com/shop/product/<product-name>?ID=<product-id>"

    def check_url_format(self):
        m = re.match(r"^http://www1.bloomingdales.com/shop/product.*$", self.product_page_url)
        return not not m
    ...
    def _product_name(self):
        return self.tree_html.xpath('//h1[@id="productTitle"]')[0].text
    ...
    def _description(self):
        description =" ".join(self.tree_html.xpath('//div[contains(@id,"ProductDescription")]//text()')).strip()
        if len(description) > 0:
            return description
        description =" ".join(self.tree_html.xpath('//div[@itemprop="description"]//text()')).strip()
        if len(description) > 0:
            return description
        description =" ".join(self.tree_html.xpath('//div[@id="pdp_tabs_body_left"]//ul/li//text()')).strip()
        if len(description) > 0:
            return description
        return None
    ...
    def _long_description(self):
        description =" ".join(self.tree_html.xpath('//div[@itemprop="description"]//text()')).strip()
        if len(description) > 0 and description != self._description() :
            return description
        l_description =" ".join(self.tree_html.xpath('//div[@id="pdp_tabs_body_left"]//ul/li//text()')).strip()
        if len(l_description) > 0 and l_description != self._description() :
...
#            print '\n\n\nImage URL:', image_url, '\n\n\n'
    ...
    def _image_urls(self, tree = None):
        if tree == None:
            tree = self.tree_html
        image_url = tree.xpath('//img[@itemprop="image"]/@src')
        all_scripts = self.tree_html.xpath('//script[@type="text/javascript"]//text()')
        image_url2 = []
        for s in all_scripts:
            st = s.find('BLOOMIES.pdp.productLvlAddImgs')
            if st >= 0:
                st = s.find('"',st)
                if st > 0:
                    fnn = s.find('BLOOMIES',st)
                    fn = s.find('";',st)
                    if fn > st+5 and fn < fnn :
                        imglst=s[st+1:fn]
                        print "img",st,fn,imglst
                        image_url2 = [r"http://images.bloomingdales.com/is/image/BLM/products/" + m.strip() for m in imglst.split(',')]
                        break

        image_url.extend(image_url2)
        if len(image_url) > 0:
            return image_url
        return None
    ...
    def _review_count(self):
        rw = self.tree_html.xpath('//meta[@itemprop="ratingCount"]/@content')
        if len(rw) > 0:
            return self._toint(rw[0])
        return None
    ...
    def _price(self):
        price = self.tree_html.xpath("//span[contains(@itemprop, 'price')]//text()")
        if price:
    ...
    def _brand(self):
        brand = self.tree_html.xpath("//div[contains(@id,'Brand')]//text()")
        if len(brand) > 0:
            return brand[0]
        return None
