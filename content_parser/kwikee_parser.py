...
def parse(sheet, dest):
    ...
    for i in range(1, sheet.nrows):
        ...
        product_json['upc'] = row[0].value

        product_json['product_name'] = row[1].value

        product_json['brand'] = row[2].value

        product_json['description'] = row[4].value

        product_json['long_description'] = '<ul>'
        for i in range(5,10):
            product_json['long_description'] += '<li>' + row[i].value + '</li>'
        product_json['long_description'] += '</ul>'

        product_json['shelf_description'] = '<ul>'
        for i in range(6,9):
            product_json['shelf_description'] += '<li>' + row[i].value + '</li>'
        product_json['shelf_description'] += '</ul>'
