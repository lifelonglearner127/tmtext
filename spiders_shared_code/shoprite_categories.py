...
class ShopriteCategoryParser:
    CATEGORIES_URL = "https://shop.shoprite.com/api/product/v5/" \
                     "categories/store/{store}?userId={user_id}"
    CATEGORY_URL = "https://shop.shoprite.com/api/product/v5/" \
                   "category/{category_id}/store/{store}/" \
    ...
    def parse_categories(self, url):
        if url:
            ...
            categories_json = json.loads(content)
            for category in categories_json:
                category_id = str(category.get('Id'))
                if category_id in self._categories:
                    category_name = category.get('Name')
                    subcategory_url = category.get(
                        'Links')[0].get('Uri') if len(category.get('Links')) > 1 else None
